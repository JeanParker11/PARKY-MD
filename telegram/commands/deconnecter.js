...
module.exports = {
  ...
  async execute(ctx) {
    ...
    console.log(`🔍 Bots à déconnecter pour ${userId}: ${userBots.length}`);
...
async function disconnectBot(ctx, botId, isGlobalDev = false) {
  try {
    const { removeSession } = require("../utils/connexion");
    const bot = botManager.getBot(botId);
    if (!bot) {
    ...
    // Supprimer du monitoring
    messageMonitor.unregisterBot(botId);
  ...
  try {
    ...
    for (const bot of botsToDisconnect) {
      try {
        ...
        messageMonitor.unregisterBot(bot.botId);
