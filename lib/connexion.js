...
// Fonction asynchrone séparée pour envoyer le message de démarrage
async function sendStartupMessage(sock, ownerJid, config) {
  try {
    await sock.sendMessage(ownerJid, { text: getStartupMessage(config) });
    console.log(`📨 Message de démarrage envoyé au propriétaire principal`);
  } catch (err) {
  ...
  sock.ev.on("connection.update", ({ connection, lastDisconnect }) => {
    ...
    if (connection === 'close') {
      ...
      if (sock.user && sock.user.id) {
        ...
        const ownerJid = `${global.owner[0] || "237693686208"}@s.whatsapp.net`;
        
        console.log(`📱 Enregistrement du bot principal ${botId}`);
        const config = botManager.registerBot(botId, sock, ownerJid);
        sock.botConfig = config;
        sock.botId = botId;
        
        messageMonitor.registerBot(botId, config.botname, ownerJid);
        
        sendStartupMessage(sock, ownerJid, config);
      }
