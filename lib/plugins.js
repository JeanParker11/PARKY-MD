...
module.exports = async (parky, m, chatUpdate, store) => {
  try {
    if (!m.message) return;
    ...
    const isOwner = botConfig ? 
      botManager.checkPermission(botConfig.botJid, sender, 'owner') :
    ...
    const isSudo = botConfig ?
      botManager.checkPermission(botConfig.botJid, sender, 'sudo') :
    ...
    if (isGroup) {
      try {
        const metadata = await getGroupMetadataSafe(parky, m.chat);
        const admins = metadata.participants.filter(p => p.admin === "admin" || p.admin === "superadmin");
        isAdmin = admins.some(p => p.id === sender);
        isBotAdmin = admins.some(p => p.id === parky.user?.id);
      } catch {
        return sendQuoted(parky, m.chat, "⚠️ Impossible de vérifier les droits du groupe pour l'instant.", m);
      }
    }
    ...
    for (const [name, command] of commands.entries()) {
      ...
      if (command.keywords.some(k => text.trim().startsWith(k))) {
        try {
          ...
          await command.execute(parky, m, args, chatUpdate, store);
        } catch (err) {
    ...
    if (text.startsWith(prefix)) {
      ...
      if (botConfig && botConfig.commands.categories[command.category] === false) {
        return sendQuoted(parky, m.chat, "❌ Cette catégorie de commandes est désactivée sur ce bot.", m);
      }
      ...
      if (command.category === "UNIROLIST") {
        if (!isGroup) {
          if (!command.allowPrivate) {
            return sendQuoted(parky, m.chat, "❌ Cette commande doit être utilisée dans un groupe UNIROLIST.", m);
          }
        } else {
          if (!unirolistGroups.includes(m.chat)) {
            return sendQuoted(parky, m.chat, "❌ Cette commande n'est pas activée dans ce groupe.", m);
          }
        }
      }
      ...
      if (!isOwner && !isGlobalDev && !command.allowedForAll) {
        if (command.onlyOwner)
          return sendQuoted(parky, m.chat, "❌ Seul le propriétaire peut utiliser cette commande.", m);

        if (command.onlySudo && !isSudo)
          return sendQuoted(parky, m.chat, "❌ Seuls les sudo peuvent utiliser cette commande.", m);

        if (command.onlyAdmin && isGroup && !isAdmin && !isSudo)
          return sendQuoted(parky, m.chat, "❌ Seuls les admins peuvent utiliser cette commande.", m);

        if (command.botAdmin && isGroup && !isBotAdmin)
          return sendQuoted(parky, m.chat, "❌ Le bot doit être admin pour cette commande.", m);

        if (!isGroup && (command.onlyAdmin || command.onlySudo || command.botAdmin))
          return sendQuoted(parky, m.chat, "❌ Cette commande est réservée aux groupes.", m);
      }

      // Mettre à jour l'activité du bot
      if (botConfig) {
        botManager.updateActivity(botConfig.botJid);
      }
      ...
      try {
        await parky.sendMessage(m.chat, { react: { text: emoji, key: m.key } });
      } catch {}

      try {
        await command.execute(parky, m, args, chatUpdate, store);
        await parky.sendMessage(m.chat, { react: { text: "", key: m.key } });
      } catch (err) {
...
function getBotConfigFromSocket(sock) {
  ...
  if (sock.botJid) return botManager.getBotConfig(sock.botJid);
  return null;
}
